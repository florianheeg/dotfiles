...
title: advanced-builtins-first
description: Prefer built-in utility types before custom type machinery
impact: LOW-MEDIUM
impactDescription: Reduces unnecessary complexity by using familiar transformations.
---

## ADV-01: Prefer built-in utility types before custom type machinery

**When to apply:** Before writing a custom mapped or conditional type: check whether `Pick`, `Omit`, `Partial`, `Record`, etc. already do the job.
